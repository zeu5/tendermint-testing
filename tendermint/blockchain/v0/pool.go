...
func NewBlockPool(start int64, requestsCh chan<- BlockRequest, errorsCh chan<- peerError, logger log.Logger) *BlockPool {
	bp := &BlockPool{
...
func (pool *BlockPool) SetPeerRange(peerID p2p.NodeID, base int64, height int64) {
	...
	if peer != nil {
		...
		pool.Logger.With("addr", peerID, "height", height).Debug("Added peer")
	...
	if height > pool.maxPeerHeight {
		pool.Logger.With("max_height", height).Debug("New Max height")
