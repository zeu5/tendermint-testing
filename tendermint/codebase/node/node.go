...
func DefaultNewNode(config *cfg.Config, logger log.Logger) (*Node, error) {
	...
	return NewNode(config,
		...
		logger,
		DefaultTransportCreator,
	)
}

func DefaultNewNodeWithTransportCreator(config *cfg.Config, logger log.Logger, creator func(*privval.FilePVKey) transportCreator) (*Node, error) {
	nodeKey, err := p2p.LoadOrGenNodeKey(config.NodeKeyFile())
	if err != nil {
		return nil, fmt.Errorf("failed to load or gen node key %s: %w", config.NodeKeyFile(), err)
	}
...
func createConsensusReactor(
	...
	consensusState := cs.NewState(
		...
		// cs.TickerOption(config.Consensus.TimeoutAsMessage),
	)
...
type transportCreator func(log.Logger, *cfg.Config, p2p.NodeInfo, string) p2p.Transport

func DefaultTransportCreator(logger log.Logger, config *cfg.Config, _ p2p.NodeInfo, _ string) p2p.Transport {
...
func NewNode(config *cfg.Config,
	...
	logger log.Logger,
	transportC transportCreator,
	options ...Option) (*Node, error) {

	blockStore, stateDB, err := initDBs(config, dbProvider)
	if err != nil {
		logger.Info("Error initializing DBs", "error", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Info("Error loading state", "error", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Info("Error creating proxyApp", "error", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Info("Error creating and starting eventbus", "error", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Info("Error creating and starting indexer service", "error", err)
		return nil, err
	}
	...
	transport := transportC(p2pLogger, config, nodeInfo, state.ChainID)
...
func (n *Node) OnStop() {
	...
	if err := n.stateStore.Close(); err != nil {
		n.Logger.Error("Error closing the state store", "err", err)
	}
	if err := n.blockStore.Close(); err != nil {
		n.Logger.Error("Error closing the block store", "err", err)
	}
}
